...
TiKV 的本地存储的 cluster id 和指定的 PD 的 cluster id 不一致。在部署新的 PD 集群的时候，PD 会随机生成一个 cluster id，TiKV 第一次的初始化的时候会从 PD 获取 cluster id 存在本地，下次启动的时候会检查本地的 cluster id 与 PD 的 cluster id 是否一致，如果不一致则会报错并推出。出现这个错误一个常见的原因是，用户原先部署了一个集群，后来把 PD 的数据删除了并且重新部署了新的 PD，但是 TiKV 还是使用旧的数据重启连到新的 PD 上，就会报这个错误。
